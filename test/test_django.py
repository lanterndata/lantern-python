...
class TestDjango:
    ...
    def test_works(self):
        ...
        assert item.id == 1
        assert np.array_equal(np.array(item.embedding), np.array([1, 2, 3] + [0] * 381))
    ...
    def test_limit(self):
        ...
        results = Item.objects.annotate(distance=distance).order_by("distance")[:1]
    ...
    def test_serialization(self):
        ...
        for format in ["json", "xml"]:
            ...
            with mock.patch(
                "django.core.serializers.python.apps.get_model"
            ) as get_model:
